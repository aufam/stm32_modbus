...
fun modbus::api::crc(const uint8_t* data, size_t len) -> uint16_t {
    uint16_t res = 0xFFFF;
...
fun modbus::api::decode(const uint8_t* data, size_t& len) -> const uint8_t* {
    if (len < 4)
        return {};

    val checksum = modbus::api::crc(data, len - 2);
    if (checksum != (data[len - 2] | data[len - 1] << 8))
        return nullptr;
